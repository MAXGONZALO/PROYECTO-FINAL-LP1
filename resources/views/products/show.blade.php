...
<div class="bg-gray-50 py-8">
    <div class="container mx-auto px-4 lg:px-6">
        ...
        <nav class="text-sm text-gray-600 mb-6">
            ...
            @if($product->category)
                <a href="{{ route('products.category', $product->category->slug) }}" class="hover:text-red-600">{{ $product->category->name }}</a> / 
            @endif
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 mb-12">
            ...
            <div>
                <div class="bg-white rounded-xl shadow-lg p-6 mb-4 sticky top-24">
                    @if($product->image)
                        ...
                        @endphp
                        <img src="{{ $imageUrl }}" alt="{{ $product->name }}" class="w-full h-96 object-contain" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                    @else
                ...
                <div class="grid grid-cols-4 gap-3">
                    ...
                    <div class="bg-white rounded-lg shadow-md p-2 cursor-pointer hover:shadow-lg transition-shadow border-2 border-transparent hover:border-red-500">
            ...
            <div>
                <div class="bg-white rounded-xl shadow-lg p-8">
                    ...
                    <div class="mb-6 pb-6 border-b border-gray-200">
                        @if($product->has_discount)
                            <div class="flex items-center gap-4 mb-3">
                                <span class="text-4xl font-bold text-red-600">S/{{ number_format($product->price, 2) }}</span>
                    ...
                    <div class="mb-6 p-4 bg-red-50 rounded-lg border border-green-100">
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 mb-8">
                        <form class="flex-1" action="{{ route('cart.buy-now', $product->slug) }}" method="POST">
                            @csrf
                            <button type="submit" class="btn-primary w-full text-lg py-4 flex items-center justify-center space-x-2">
                        ...
                        <form class="flex-1" action="{{ route('cart.add', $product->slug) }}" method="POST">
                            @csrf
                            <button type="submit" class="btn-secondary w-full text-lg py-4 flex items-center justify-center space-x-2">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="product-card">
                    <a href="{{ route('products.show', $related->slug) }}">
                        ...
                        <div class="p-5">
                            ...
                            <p class="text-2xl font-bold text-red-600 mb-4">S/{{ number_format($related->price, 2) }}</p>
